...
void setup() 
{
  ...
  if (!initUSB(&usb))
  {
    initUSB(&usb);                           //Try one more time to initialize USB
  ...
  OCR1A = 15624;// 15624 = (16*10^6) / (1*1024) - 1 (must be <65536) (15624 is 1 Hz)
  // turn on CTC mode
  TCCR1B |= (1 << WGM12);
...
//Interrupt service routine (ISR) for timer 1 - sensor collection
ISR(TIMER1_COMPA_vect)
{
  noInterrupts();    //Turn interrupts off while we handle global variables
  float volts = 0.0;
  float cm = 0.0;
  ...
  interrupts();                     //Re-enable interrupts for I2C to be usable
  ...
  noInterrupts();                        // Disable interrupts as we don't need them right now
  // print out data
  ...
  interrupts();  //We are done.  Turn interrupts back on.
}
...
void loop() 
{
  //Interrupt driven...loop forever with nothing to do
}
