...
String data1,data2,check = "";                               // variables to send to Serial 0 and 1
bool usb_status = 0;                                         // USB status is on or off
...
void setup() 
{
  ...
  while(!usb_status && (usb_fail_count < 6))
  {
    usb_status = initUSB(&usb);
    if(usb_status == 0)
    {
  ...
  //Wait for data from Serial1
  while(!Serial1.available()){}
 
  //Make sure ESP is connected to Internet and Google Sheets
  while(check != "Connected\r")
  {
    delay(5);
...
//Appends data to a USB flash drive file
void fileApp(String buf, File f)
{
...
void loop() 
{
  if(check == "Success")  //Only take a sample when ESP is ready
  {
    ...
    //Format data for flash drive
    data1=("aX = ")+accelerometer_xS+(" aY = ")+accelerometer_yS+(" aZ = ")+accelerometer_zS+(" tmp = ")+temperatureS+(" gX = ")+gyro_xS+(" gY = ")+gyro_yS+(" gZ = ")+gyro_zS+(" Distance = ")+result+('\n');

    //Format data for Google Sheets JSON
    accelerometer_xS.trim();
    accelerometer_yS.trim();
    accelerometer_zS.trim();
    temperatureS.trim();
    gyro_xS.trim();
    gyro_yS.trim();
    gyro_zS.trim();
    data2 = "\"" + accelerometer_xS+(",")+accelerometer_yS+(",")+accelerometer_zS+(",")+temperatureS+(",")+gyro_xS+(",")+gyro_yS+(",")+gyro_zS+(",")+result+ "\"}" + ('\n');  
      
    Serial.print(data2);                //Send data to Terminal
    if(usb_status)
    {
      fileApp(data1, file);               //Write data to flash drive
    }
    delay(10);
    Serial1.print(data2);               //Send data to ESP
    delay(5000);                        //Need to wait for Google Sheets
    
    while(!Serial1.available()){}       //Wait for ESP to send something
    
    while(check != "Success")           //Read until ESP sends confirmation
    {
      delay(5);
      check = Serial1.readStringUntil('\n');
      check.trim();
    }
    check = "Success";                  //Make sure check is written as the loop expects
  }
}
